...
lvl0 = str(open('LVL0.txt', encoding="utf-8").read().strip()).split('\n')
lvl1 = str(open('LVL1.txt', encoding="utf-8").read().strip()).split('\n')
lvl2 = str(open('LVL2.txt', encoding="utf-8").read().strip()).split('\n')
lvl3 = str(open('LVL3.txt', encoding="utf-8").read().strip()).split('\n')
lvl4 = str(open('end_screen.txt', encoding="utf-8").read().strip()).split('\n')
for el in range(len(lvl0)):
    lvl0[el] = lvl0[el].split(', ')
for el in range(len(lvl1)):
    lvl1[el] = lvl1[el].split(', ')
for el in range(len(lvl2)):
    lvl2[el] = lvl2[el].split(', ')
for el in range(len(lvl3)):
    lvl3[el] = lvl3[el].split(', ')
for el in range(len(lvl4)):
    lvl4[el] = lvl4[el].split(', ')

LEVELS = {'0': lvl0, '1': lvl1, '2': lvl2, '3': lvl3, '4': lvl4}
...
def jump():
    ...
    if jump_count >= -11:
        if z_down and jump_count >= 0:
            y -= 2
        if jump_count < 0:
            if check_place(x, y + (jump_count ** 2) / 4) == 'on the floor':
                is_jump = False
                jump_count = 10
                can_jump = True
                while check_place(x, y) != 'on the floor':
                    y += 1
                return
            y += (jump_count ** 2) / 4
        else:
    ...
    # if check_place(x, y) != 'on the floor' and jump_count < -11:
    #     jump_count += 1
...
def check_place(x_, y_):
    ...
    x__, y__ = get_coords()
    try:
        if lvl[int(y_ // 20)][int(x_ // 20)].strip() == 'EXIT' or\
                lvl[int(y_ // 20)][int(x_ // 20) + 1].strip() == 'EXIT':
            return 'EXIT'
        ans = ''
    ...
    try:
        if lvl[int(y__ + 1)][int(x__)].strip() == 'TRAP' or lvl[int(y__)][int(x__)].strip() == 'TRAP'\
                or (lvl[int(y__ )][int(x__ + 1)].strip() == 'TRAP' or lvl[int(y__)][int(x__ - 1)].strip() == 'TRAP'):
            return 'on the trap'
    ...
    for y_ in range(40):
        for x_ in range(40):
            if LEVELS[str(lvl_now)][y_][x_].strip() == 'FLOOR':
                ...
                pygame.draw.rect(screen, pygame.Color(choice(colors)), (x_ * 20, y_ * 20, 20, 20), 0)
            elif LEVELS[str(lvl_now)][y_][x_].strip() == 'EXIT':
                pygame.draw.rect(screen, pygame.Color(0, 255, 0), (x_ * 20, y_ * 20, 20, 20), 0)
...
def check_coords(x_, y_):
    ...
    try:
        ...
        while pos_down != '0':
            ...
            pos_down = LEVELS[str(lvl_now)][int((y_ + 20) // 20)][int(x_ // 20)].strip()
            # print(y_, pos_down)
        if LEVELS[str(lvl_now)][int((y_ + 20) // 20)][int(x_ // 20) + 1].strip() == 'FLOOR':
            while LEVELS[str(lvl_now)][int((y_ + 20) // 20)][int(x_ // 20) + 1].strip() != '0':
                y_ -= 1
        if LEVELS[str(lvl_now)][int((y_ + 21) // 20)][int(x_ // 20)].strip() == 'FLOOR' or \
            LEVELS[str(lvl_now)][int((y_ + 21) // 20)][int((x_ + 20) // 20)].strip() == 'FLOOR':
            y_ += 1

        if place == 'on the floor' and pos == 'FLOOR':
            y_ = y_ // 20 * 21

        if place == 'on the trap' or LEVELS[str(lvl_now)][int((y_ + 22) // 20)][int(x_ // 20)].strip() == 'TRAP' or \
                LEVELS[str(lvl_now)][int(y_  // 20)][int(x_ // 20)].strip() == 'TRAP':
            x_, y_ = 0, 760
            deaths += 1
